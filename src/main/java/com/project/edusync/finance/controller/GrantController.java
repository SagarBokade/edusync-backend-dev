...
/** /auth/finance/grants */
@RestController @RequestMapping("/auth/finance/grants") @RequiredArgsConstructor
public class GrantController {

    private final GrantServiceImpl grantService;
    private static final Long SID = 1L;

    @GetMapping @PreAuthorize("hasAnyAuthority('finance:grants:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    public ResponseEntity<List<GrantResponseDTO>> getAll() { return ResponseEntity.ok(grantService.getAllGrants(SID)); }

    @GetMapping("/{id}") @PreAuthorize("hasAnyAuthority('finance:grants:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @GetMapping("/nearing-expiry") @PreAuthorize("hasAnyAuthority('finance:grants:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PutMapping("/{id}") @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/activate") @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/status") @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/close") @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/lapse") @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    public ResponseEntity<GrantResponseDTO> lapse(@PathVariable Long id) { return ResponseEntity.ok(grantService.lapsGrant(id, SID)); }

    // Utilisation
    @PostMapping("/utilisations") @PreAuthorize("hasAnyAuthority('finance:grants:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @GetMapping("/{id}/utilisations") @PreAuthorize("hasAnyAuthority('finance:grants:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
