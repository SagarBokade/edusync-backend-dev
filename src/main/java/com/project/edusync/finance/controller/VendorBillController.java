...
@RestController @RequestMapping("/auth/finance/vendor-bills") @RequiredArgsConstructor
public class VendorBillController {
    private final VendorBillServiceImpl billService;
    private static final Long SID = 1L;

    @GetMapping @PreAuthorize("hasAnyAuthority('finance:ap:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @GetMapping("/{id}") @PreAuthorize("hasAnyAuthority('finance:ap:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @GetMapping("/overdue") @PreAuthorize("hasAnyAuthority('finance:ap:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    public ResponseEntity<List<VendorBillResponseDTO>> getOverdue() {
        return ResponseEntity.ok(billService.getOverdueBills(SID));
    }

    @GetMapping("/outstanding-payables") @PreAuthorize("hasAnyAuthority('finance:ap:read','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    public ResponseEntity<BigDecimal> getOutstandingPayables() {
        return ResponseEntity.ok(billService.getOutstandingPayables(SID));
    }

    @PostMapping @PreAuthorize("hasAnyAuthority('finance:ap:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/approve-payment") @PreAuthorize("hasAnyAuthority('finance:ap:approve','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/record-payment") @PreAuthorize("hasAnyAuthority('finance:ap:approve','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/override-mismatch") @PreAuthorize("hasAnyAuthority('finance:ap:approve','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
    ...
    @PostMapping("/{id}/cancel") @PreAuthorize("hasAnyAuthority('finance:ap:write','ROLE_ADMIN','ROLE_FINANCE_ADMIN')")
